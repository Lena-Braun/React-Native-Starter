...
export default function AvatarUploader({ fieldchange, mediaUrl = '', firstName, lastName, username, country, city }: AvatarUploaderProps) {
    ...
    const [isImageSelected, setIsImageSelected] = useState<boolean>(fileUrl !== '')
    ...
    return (
        <div className="flex items-start max-sm:gap-6 sm:gap-10">
            ...
            <div className="flex flex-col justify-start gap-4">
                ...
                <p className="text-sm text-stone-500"> {city} {city ? ',' : ''} {country} </p>
                <button {...getRootProps()} type='button' className="bg-orangeRed mt-3 px-3 py-2 text-white text-sm rounded-md"> Change Avatar </button>
            </div>
